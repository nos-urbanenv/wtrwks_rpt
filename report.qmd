...
GLC_bel_lines <- GLC_bel_lines %>% 
  stringr::str_split('\t')
GW_bel_lines <- readLines("GW1203.txt")
GW_bel_lines <- GW_bel_lines %>% 
  stringr::str_split('\t')

GLC_bel_lines[37:83] %>% unlist() %>% matrix(ncol=6,byrow = TRUE)
GLC_bel_lines[37:83] %>% unlist() %>% matrix(ncol=6,byrow = TRUE) %>% as.table() %>% t() %>% View()
