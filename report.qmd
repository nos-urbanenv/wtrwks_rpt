...
上記の実測値より、各マノメータごとの損失水頭を求めた結果を @tbl-lost-head に示す。なお、 @tbl-manometer に示した通り、マノメータの7番と8番は濾過プロセス全体を通してほぼ同一の値で水位している。これは、7番と8番のマノメータが濾過層よりも上部に設置されているためである。そこで、損失水頭を求めるに当たっては、7番と8番のマノメータの読みの値の平均値から、1\~6番のマノメータ水位を引くことによって求めた。
...
manometer_losthead %>% 
  ...
  ggplot(aes(x=value,y=manometer,color=sampling))+
  geom_point()+
  geom_line(linetype=3)+
  scale_y_reverse()+
