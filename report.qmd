...
inverse_pred <- function(y, lm_mod){
  ...
  if(is.na(y)){
    ...
    return(tibble(MB_conc=NA, MB_info="検出下限"))
  }
  else if(pred.val > upper_lim){
    return(tibble(MB_conc=pred.val, MB_info="外挿"))
  }
  else{
    return(tibble(MB_conc=pred.val, MB_info="検量線範囲内"))
  }
}
...
次に、複数の希釈列を作成し、665nm吸光度を測定すると @fig-abs-calib のような検量線データが得られた。なお、私達3班は2日目の測定時に新しく検量線を作成していたが、4班は作成していなかったとのことなので、 @fig-abs-calib には3種類の検量線データについて図示した。また、各検量線の回帰式とR\^2値を @tbl-abs-models に示した。

```{r fig-abs-calib, dev='jpeg'}
#| echo: false
#| message: false
#| warning: false
#| fig-cap: "メチレンブルー濃度の検量線"
...
<!--# リンクを貼るのを忘れずに！ QuartoやRのパッケージにもリンクを貼り、引用文献扱いとしよう。-->

本レポートは科学技術出版システム[Quarto](https://quarto.org/)を用いて作成しました。表組み・図の作成は、Quarto Document内にR言語のスクリプトを埋め込むことで自動化しています（図表作成には[ggplot2](https://ggplot2.tidyverse.org/)、[gt](https://gt.rstudio.com/index.html)パッケージを使用）。この方法には、実験データが差し替えとなった場合でも図表を再生成することができることや、実験ノートからの転記作業を除いてExcelによる手作業が入り込む余地が一切なくなった結果、データ処理過程の検証・修正がしやすくなることなどの利点があります。

<!--# (もしできたならこの文言を入れる) 今回は図表生成をスクリプトで完全に自動化することで、自班以外のデータや、分析機器の生データを積極的に取り入れた考察を行うことが可能となりました。 -->

<!--# また、BELSORP miniの計測結果の処理もドキュメント内に組み込んであります。Belsorpの計測結果は、実験者が指定したディレクトリに"GLC1203.DAT"のようなファイル名のテキストファイルとして保存されているのを発見しました。今回はそのDATファイルから取り出した、圧力と体積の実測データを使用して、解析を行いました。 -->
